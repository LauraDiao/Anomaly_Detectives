...
def classify_(
    ...
    model_eda()
    lossmodel = gen_model(
        "loss", n_jobs, train_window, pca_components, test_size, verbose
    )
    latmodel = gen_model(
        "latency", n_jobs, train_window, pca_components, test_size, verbose
    )
    lossp, latp = performance_metrics(
        "data/temp/tempdata_c",
        lossmodel,
        latmodel,
        loss_thresh,
...
def main(targets):
    ...
    if "test" in targets:
        ...
        data(
            transform_config, test_unseen, test_seen, is_test=True
        )  # runs on test dataset
        eda(
            eda_config, test_seen, is_test=True
        )  # ignores some viz that doesnt work with test dataset
